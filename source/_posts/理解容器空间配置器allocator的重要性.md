...
int main()
{
    vector<Test> vec;   //空容器，但输出发现构造、析构被调用很多次，因为vector构造时new开辟加实例化
...
``` c++
//这里实现空间配置器allocator,内存开辟/内存释放/对象构造/对象析构
template <typename T>
...
public:
    ...
    {
        new(p)T(value); //定位new，在指定位置p处创建对象value

    }
    void destroy(T* p)      //析构元素
    {
        p->~T();    //~T()代表T类型的的析构函数，调用p的析构函数
    }
...
namespace myspace
{
template <typename T,typename Alloc=Allocator<T>> //这后面的两个是形参，
class vector
{
    public:
    vector(int size=10)
